...
def _selection_workbook_action(paths: Iterable[Path | str]) -> str:
    """Return how the RIGHT workbook pane should react to LEFT selection.

    A single Excel file is displayable (``"load"``).  A single directory or
    non-Excel file means the previous workbook is stale and must be cleared
    (``"clear"``).  Zero/multi-selection keeps the current multi-select workflow
    unchanged (``"keep"``).
    """
...
class EditableWorkbookModel(WorkbookModel):
    ...
    def _load_editable(self) -> None:
        ...
        if suffix == ".xls":
            ...
            self.workbook = workbook
            self.sheets = [
                ReadOnlyXlrdSheetAdapter(workbook.sheet_by_index(i))
                for i in range(workbook.nsheets)
            ]
            self.kind = "Legacy Excel (.xls) - read only"
            self.editable = False
            return
    ...
    def save(self, target: Optional[Path] = None, *, create_backup: bool = True) -> Path:
        if not self.editable or self.workbook is None:
            raise RuntimeError("This workbook is read-only in xExcel Viewer.")
...
class ExcelWorkspaceApp(tk.Tk):
    ...
    def __init__(self, initial_path: Optional[Path] = None) -> None:
        super().__init__()
        self.title(f"xExcel Viewer {APP_VERSION}")
        ...
        self.excel_only = tk.BooleanVar(value=True)
        self.zoom_value = tk.StringVar(value="100%")
        self.status_text = tk.StringVar(value="Select an Excel file on the left. Its full workbook will appear on the right.")
        self.cell_ref = tk.StringVar(value="A1")
        self.formula_value = tk.StringVar(value="")
        self.workbook_title = tk.StringVar(value="No workbook selected")
        ...
        if initial_path is not None and initial_path.is_file() and initial_path.suffix.lower() in EXCEL_EXTENSIONS:
            self.after(150, lambda: self.open_workbook(initial_path))

    def _set_icon(self) -> None:
        try:
            icon_path = Path(__file__).resolve().parent / "assets" / "xexcel.ico"
    ...
    def _build_ui(self) -> None:
        ...
        left = ttk.LabelFrame(pane, text="LEFT — Excel files", padding=(5, 4))
        right = ttk.LabelFrame(pane, text="RIGHT — Full workbook / cells", padding=(5, 4))
        ...
        left_tools.pack(fill="x", pady=(0, 4))
        ttk.Label(left_tools, text="Files", font=("Segoe UI", 10, "bold")).pack(side="left")
        ttk.Checkbutton(
            left_tools, text="Excel only", variable=self.excel_only,
            command=lambda: self.refresh_files(rescan=False)
        ...
        ttk.Button(editor_toolbar, text="Copy", command=self.copy_current).pack(side="left", padx=(8, 2))
        ttk.Button(editor_toolbar, text="Paste", command=self.paste_current).pack(side="left", padx=2)
        ttk.Button(editor_toolbar, text="Copy Image", command=self.copy_image_current).pack(side="left", padx=2)
        ttk.Button(editor_toolbar, text="Open in Excel", command=self.open_default).pack(side="left", padx=(8, 2))
        ...
        ttk.Label(
            self.blank,
            text=(
                "Select an Excel file on the left.\n\n"
                "The complete workbook will open here.\n"
                "Ctrl+Left goes to FILES; Ctrl+Right goes to WORKBOOK.\n"
                "Alt+1 jumps to FILES; Alt+2 jumps to WORKBOOK.\n"
                "Arrow keys select files on the left and move cells on the right.\n"
                "Tab / Shift+Tab moves to the next / previous Excel cell.\n"
                "All worksheet names are available in the scrollable Sheets bar above."
            ),
            anchor="center",
            justify="center",
            font=("Segoe UI", 13),
    ...
    def _set_panel_indicator(self, active: str) -> None:
        try:
            self.left_panel.configure(text="LEFT — Excel files  [ACTIVE]" if active == "left" else "LEFT — Excel files")
            self.right_panel.configure(text="RIGHT — Full workbook / cells  [ACTIVE]" if active == "right" else "RIGHT — Full workbook / cells")
        except Exception:
            pass
    ...
    def _activate_left_panel(self) -> None:
        ...
        self.status_text.set("ACTIVE: LEFT FILES | Up/Down selects a workbook | Ctrl+Right = WORKBOOK | Alt+2 = WORKBOOK")
    ...
    def _run_link_action(self, link: LinkDefinition) -> None:
        ...
        if action == "powershell_here":
            ...
            self.status_text.set("Hidden/System toggle is not exposed in xExcel Viewer.")
        else:
    ...
    def refresh_files(self, *, rescan: bool = True) -> None:
        ...
        for rec in self._directory_records:
            ...
            if self.excel_only.get() and not is_dir and path.suffix.lower() not in EXCEL_EXTENSIONS:
                continue
        ...
        if filter_text:
            ...
            self.status_text.set(
                f"{self.current_dir} | {len(records):,} item(s) | Ctrl+F searches filenames | Search Up/Down moves to files | Right-drag selects multiple | Select an Excel file to show the full workbook on the right"
            )
    ...
    def _file_selected(self, _event=None) -> None:
        ...
        # A directory or non-Excel file cannot be represented by the RIGHT
        # workbook pane.  Clear the old workbook immediately so its last image
        # never looks like it belongs to the newly selected item.  Invalidate
        # scheduled/in-flight loaders as part of the clear.  Unsaved edits are
        # protected with the same Save / Discard / Cancel flow used when moving
        ...
        if path.is_dir():
            self.status_text.set(
                f"{path.name} | Folder selected. Press Enter or double-click to open it. RIGHT workbook cleared."
            )
        elif path.is_file():
            self.status_text.set(
                f"{path.name} | Not an Excel workbook. Press Enter or double-click to open with the default application. RIGHT workbook cleared."
            )
        else:
            self.status_text.set(f"{path.name} | Item is no longer available. RIGHT workbook cleared.")
    ...
    def _file_activated(self, _event=None) -> None:
        """Mouse double-click keeps the integrated xExcel activation behavior."""
        path = self.selected_file_path()
        if path is None:
            return
        if path.is_dir():
    ...
    def _load_complete(self, path: Path, generation: int, model: EditableWorkbookModel) -> None:
        ...
        self.title(f"xExcel Viewer {APP_VERSION} — {path.name}")
    ...
    def save_workbook(self) -> bool:
        ...
        self.title(f"xExcel Viewer {APP_VERSION} — {target.name}")
    ...
    def save_as(self) -> bool:
        ...
        self.title(f"xExcel Viewer {APP_VERSION} — {saved.name}")
...
def self_check() -> int:
    try:
        ...
        print(f"xExcel Viewer self-check failed: {exc}", file=sys.stderr)
        return 1
    try:
    ...
    print(f"xExcel Viewer {APP_VERSION} self-check OK ({legacy})")
    return 0
